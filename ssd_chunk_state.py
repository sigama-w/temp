...
def _chunk_cumsum_fwd_kernel(
    ...
    BLOCK_SIZE_H: tl.constexpr = 2, ### 这里原始是16会报uboverlfow
):
...
def _chunk_cumsum_fwd(
    ...
    with torch.npu.device(dt.device.index):  ### 这里原始 with torch.cuda.device(dt.device.index):
...
def _chunk_state_fwd(
    ...
    with torch.npu.device(x.device.index):  ### 这里原始 with torch.cuda.device(dt.device.index):
